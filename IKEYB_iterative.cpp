// commented out code -- gives us 2nd variation for storing only count of letters at each key
// but that it not best practice, since it becomes difficult then to reconstruct that path/backtracking.
...
void do_testcase()
{
    ...
    for (int j = K; j > 0; j--)
    {
        ...
        // letters_per_key[j - 1] = dp[i][j].i_prev;
        i = dp[i][j].i_prev;
        // i -= dp[i][j].i_prev;
    }
    int j = 0;
